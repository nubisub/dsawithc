...
// of the tree.The function also prints height of every node void
preOrder(struct Node *root)
{
	if (root != NULL)
	{
		printf("%d,%s ", root->key, root->nama);
		preOrder(root->left);
		preOrder(root->right);
	}
}

displayInorder(struct Node *root)
{
	if (root == NULL)
		return;
...
displayPostorder(struct Node *root)
{
	if (root == NULL)
		return;
...
int main()
{
	...
	root = insert(root, 9, "a");
	root = insert(root, 5, "b");
	root = insert(root, 10, "c");
	root = insert(root, 0, "d");
	root = insert(root, 6, "e");
	root = insert(root, 11, "f");
	root = insert(root, -1, "g");
	root = insert(root, 1, "i");
	root = insert(root, 2, "j");
